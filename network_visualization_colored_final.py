...
# Unisci i cluster per nodi sia Source che Target
...
# Centralità + colori per cluster
centrality = nx.degree_centrality(G)
clusters = list(set(node_cluster_map.values()))
...
plt.title('Rete keyword - cluster colorati')
plt.axis('off')
plt.tight_layout()
plt.savefig('network_static_colored_fixed.png')
plt.show()
...
# Set opzioni manuali per visualizzazione
net.set_options("""
const options = {
  nodes: {
    font: { size: 16 }
...
for node in G.nodes():
    ...
    net.add_node(node, label=node, title=f"Cluster: {cl}\nCentralità: {centrality[node]:.3f}",
                 size=10 + 40 * centrality[node], color=cluster_colors.get(cl, '#cccccc'))

for src, tgt, data in G.edges(data=True):
    net.add_edge(src, tgt, value=data['weight'])

net.write_html('network_interactive_colored_fixed.html')
print("✅ File 'network_static_colored_fixed.png' e 'network_interactive_colored_fixed.html' generati con successo.")
